...
File: src/NLI_KD_training.py
 - Run training and evaluating for both simple finetuning teacher and patience finetuning student.
...
from utils.argument_parser import default_parser, complete_argument
from utils.data_processing import init_model, get_task_dataloader
from utils.get_loss import distillation_loss, patience_loss
from utils.load_student import lobert_student
from utils.modeling import BertForSequenceClassificationEncoder, FullFCClassifierForSequenceClassification
from utils.nli_data_processing import processors, output_modes
from utils.util import load_model, count_parameters, eval_model_dataloader_nli, eval_model_dataloader
...
args.raw_data_dir = os.path.join(HOME_DATA_FOLDER, 'data_raw/glue_data', args.task_name)
...
################################################################################
# This part has been changed from the original script at PKD!
################################################################################
elif args.kd_model.lower() == 'lobert':
    logger.info('using normal Knowledge Distillation for lobert')
    
    output_all_layers = True
    student_encoder, student_classifier = init_model(task_name, output_all_layers, args.student_hidden_layers, student_config)
    
    n_student_layer = len(student_encoder.bert.encoder.layer)
    student_encoder = lobert_student(student_config, student_encoder, args.encoder_checkpoint, args, student_mode = args.lobert_mode, num_hidden_layers = n_student_layer)
    logger.info('*' * 77)
    student_classifier = load_model(student_classifier, args.cls_checkpoint, args, 'classifier', verbose=True)
################################################################################
...
if args.do_train:
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            full_output, pooled_output = student_encoder(input_ids, segment_ids, input_mask) #, layer_idx = [0,2,4,6,8,10])
            if args.kd_model.lower() in['kd', 'kd.cls']:
...
#             logger.info(f'loss_dl: {loss_dl}, kd_loss: {kd_loss}, ce_loss: {ce_loss}')
            if args.beta > 0:
